...
static s32 find_wall_collisions_from_list(struct SurfaceNode *surfaceNode,
    ...
    while (surfaceNode != NULL) {
        ...
        data->x = x = x + surf->normal.x * (radius - offset);
        data->z = z = z + surf->normal.z * (radius - offset);
...
s32 find_wall_collisions(struct WallCollisionData *colData) {
    ...
    cellX = ((x + LEVEL_BOUNDARY_MAX) / CELL_SIZE) & CELLS_BITS;
    cellZ = ((z + LEVEL_BOUNDARY_MAX) / CELL_SIZE) & CELLS_BITS;
...
f32 unused_find_dynamic_floor(f32 xPos, f32 yPos, f32 zPos, struct Surface **pfloor) {
    ...
    // Each level is split into cells to limit load, find the appropriate cell.
    s16 cellX = ((x + LEVEL_BOUNDARY_MAX) / CELL_SIZE) & CELLS_BITS;
    s16 cellZ = ((z + LEVEL_BOUNDARY_MAX) / CELL_SIZE) & CELLS_BITS;
...
void debug_surface_list_info(f32 xPos, f32 zPos) {
    ...
    list = gStaticSurfacePartition[cellZ & CELLS_BITS][cellX & CELLS_BITS][SPATIAL_PARTITION_FLOORS].next;
    numFloors += surface_list_length(list);

    list = gDynamicSurfacePartition[cellZ & CELLS_BITS][cellX & CELLS_BITS][SPATIAL_PARTITION_FLOORS].next;
    numFloors += surface_list_length(list);

    list = gStaticSurfacePartition[cellZ & CELLS_BITS][cellX & CELLS_BITS][SPATIAL_PARTITION_WALLS].next;
    numWalls += surface_list_length(list);

    list = gDynamicSurfacePartition[cellZ & CELLS_BITS][cellX & CELLS_BITS][SPATIAL_PARTITION_WALLS].next;
    numWalls += surface_list_length(list);

    list = gStaticSurfacePartition[cellZ & CELLS_BITS][cellX & CELLS_BITS][SPATIAL_PARTITION_CEILS].next;
    numCeils += surface_list_length(list);

    list = gDynamicSurfacePartition[cellZ & CELLS_BITS][cellX & CELLS_BITS][SPATIAL_PARTITION_CEILS].next;
    numCeils += surface_list_length(list);
...
{
	// Skip if OOB
	if (cellX >= 0 && cellX <= CELLS_BITS && cellZ >= 0 && cellZ <= CELLS_BITS)
	{
