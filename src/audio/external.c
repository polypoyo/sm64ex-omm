...
u8 func_803200E4(u16 fadeTimer) {
    ...
    if (gSequencePlayers[SEQ_PLAYER_LEVEL].enabled == TRUE) {
        if (vol != 0xff) {
            ...
            gSequencePlayers[SEQ_PLAYER_LEVEL].volume = BACKGROUND_MUSIC_DEFAULT_VOLUME_FLOAT;
#endif
...
void set_sequence_player_volume(s32 player, f32 volume) {
    ...
    if (player == SEQ_PLAYER_LEVEL && sPlayer0CurSeqId != 0xff) {
        struct SequencePlayer *seqPlayer = &gSequencePlayers[player];
        seqPlayer->volume = MIN(seqPlayer->volume, BACKGROUND_MUSIC_DEFAULT_VOLUME_FLOAT);
        seqPlayer->fadeVolume = MIN(seqPlayer->fadeVolume, BACKGROUND_MUSIC_DEFAULT_VOLUME_FLOAT);
